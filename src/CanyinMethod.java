...
public class CanyinMethod {
    ...
    public void getLocationSet(Mat src, Point[][] pointSet) {
        ...
        Imgproc.threshold(gray, gray, 110, 255, Imgproc.THRESH_BINARY);
    ...
    public void getTicket(Mat src, Point[][] pointSet) {
        for (Point[] points : pointSet) {
            if (points[0] != null) {
                ...
                String filename = "./images/canyinFile" + index + ".png";
                index++;
    ...
    private Point[] getRect(Point[] points) {
        ...
        double right_y = (points[0].y - points[2].y) * 6.5 + points[2].y;
        double right_x = (points[0].x - points[2].x) * 6.5 + points[2].x;

        double left_y = (points[2].y - points[0].y) * 2.5 + points[0].y;
        double left_x = (points[2].x - points[0].x) * 2.5 + points[0].x;

        double up_y = (points[0].y - points[1].y) * 17 + points[1].y;
        double up_x = (points[0].x - points[1].x) * 17 + points[1].x;

        double down_y = (points[1].y - points[0].y) * 2 + points[0].y;
        double down_x = (points[1].x - points[0].x) * 2 + points[0].x;
